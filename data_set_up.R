# Install helper packages (will force a restart)
...
# confirm rtools is exists
pkgbuild::find_rtools(debug = TRUE)

#

# load the dataset and clean however you like
nm_2011_2024 <- read.csv("C:/Users/Jesus/Box/Energy/net_metering/data/clean data/df_1_utilty_by_month_estimation_ready_set_all_report.csv")

colnames(nm_2011_2024)
# select the following variables
nm_2011_2024 <- nm_2011_2024 %>%
  select(year,month,state,utility,utility_id,ownership,service_type,TOU,solar_esb_mwh,solar_inmc_mw,solar_nm_customers,
         wind_esb_mwh,wind_inmc_mw,wind_nm_customers, total_esb_mwh,total_inmc_mw,total_nm_customers,
         cloudy_days,avg_tcc,central_longitude,central_lattitude,avg_temp_c,avg_radiation_j_m2,med_inc,avg_inc,
         sd_dev_med_inc,revenue_1k_usd,sales_mwh,customers,avg_price_cents_per_kwh,retail_neg,avoided_cost_neg,fit_neg)

# Step 1: Create a new package
#create_package("net.metering")  # already done but you can verify using this: usethis::proj_get()

# set the project
# setwd("C:/Users/Jesus/OneDrive/Documents/net.metering")
# usethis::proj_set("C:/Users/Jesus/OneDrive/Documents/net.metering")
...
# Save dataset in data/ as .rda
use_data(nm_2011_2024, overwrite = TRUE)

# Step 3: Document the dataset (usethis::use_r("nm_2011_2024"))
# In R/nm_2011_2024.R, add:
#' Net Metering Data by Utility, 2011–2024
#'
#' Monthly panel data on energy sold back to utility companies through net
...
#'     utility by net metering customers, in megawatt-hours (MWh).}
#'   \item{solar_inmc_mw}{Numeric. Installed net metering capacity (INMC)
#'     for solar, in megawatts (MW).}
#'   \item{solar_nm_customers}{Integer. Number of customers enrolled in net
#'     metering for solar.}
#'   \item{wind_esb_mwh}{Numeric. Wind energy sold back (ESB) to the utility
#'     by net metering customers, in megawatt-hours (MWh).}
#'   \item{wind_inmc_mw}{Numeric. Installed net metering capacity (INMC) for
#'     wind, in megawatts (MW).}
#'   \item{wind_nm_customers}{Integer. Number of customers enrolled in net
#'     metering for wind.}
#'   \item{total_esb_mwh}{Numeric. Total energy sold back (ESB) to the
#'     utility across all sources, in megawatt-hours (MWh).}
#'   \item{total_inmc_mw}{Numeric. Total installed net metering capacity
#'     (INMC) across all sources, in megawatts (MW).}
#'   \item{total_nm_customers}{Integer. Total number of customers enrolled
